...
async def test_flush_logs_with_records(s3_handler, sample_log_records):
    ...
    s3_handler.log_buffer = [
        s3_handler.format_log_entry(record)
    ...
    with patch('socket.gethostname', return_value='test-host'), \
            ...
            patch.object(s3_handler.session, 'client', return_value=mock_s3_context):
        await s3_handler._flush_logs()

        # Verify S3 put_object was called with correct parameters
        expected_key = f"application-logs/date=2024-01-01/hour=12/host=test-host/2024-01-01T12:00:00_test-host_12345.jsonl.gz"
...
async def test_flush_logs_handles_s3_error(s3_handler, sample_log_records):
    ...
    initial_buffer_size = len(s3_handler.log_buffer)
    ...
    with patch('builtins.print') as mock_print:
        with patch.object(s3_handler.session, 'client', return_value=mock_s3_context):
            ...
            mock_print.assert_called_with("Error uploading logs to S3: S3 Upload Failed")
